...
def hyn0_predict(
	...
	if hybrid_model_type == 2:
		...
		videofilename=videofilename, video_bitrate=video_bitrate, video_width=video_width, video_height=video_height, video_framerate=video_framerate, re_encoded_video=re_encoded_video)
		...
		if video_codec == "h264":
			...
			per_second_scores = []
			prediction["per_sequence"] = 0.90534066 * prediction["per_sequence"] + 0.09309030
		...
		if video_codec == "vp9":
			per_sequence_transcoded = prediction["per_sequence"]
			per_second_scores = []
			prediction["per_sequence"] = 0.85302496 * prediction["per_sequence"] + 0.69794354
			for per_second_score in prediction["per_second"]:
				per_second_scores.append((per_second_score / per_sequence_transcoded) * prediction["per_sequence"])
			prediction["per_second"] = per_second_scores
	else:
...
def re_encoding(video, video_bitrate, video_width, video_height, video_framerate, video_codec, output_folder, hybrid_model_type):
	...
	if hybrid_model_type:
		cmd = """ffmpeg -nostdin -loglevel quite -threads 4 -y -i {video} -c:v libx265 -b:v {video_bitrate}k -vf scale="{video_width}:{video_height}" -r {video_framerate} -pix_fmt yuv420p -an {re_encoded_video} 2>/dev/null"""
	else:
		cmd = """ffmpeg -nostdin -loglevel quite -threads 4 -y -i {video} -c:v {video_codec} -b:v {video_bitrate}k -vf scale="{video_width}:{video_height}" -r {video_framerate} -pix_fmt yuv420p -an {re_encoded_video} 2>/dev/null"""
	shell_call(cmd)
